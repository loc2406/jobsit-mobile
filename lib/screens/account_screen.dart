...
class _AccountScreenState extends State<AccountScreen> {
  ...
  Future<void> downloadAndSaveImage(String imageUrl, String fileName) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        String filePath = await saveImage(imageBytes, fileName);
        setState(() {
          _localAvatarPath = filePath;
        });
        //print("Ảnh đã lưu tại: $filePath");
      } else {
        print("Lỗi tải ảnh: ${response.statusCode}");
      }
    } catch (e) {
      print("Lỗi: $e");
    }
  ...
  Widget build(BuildContext context) {
    ...
    email = args?[TextConstants.emailUser];
    password = args?[TextConstants.passwordUser];
    return Scaffold(
      appBar: AppBar(
      ...
      body: BlocBuilder<CandidateCubit, CandidateState>(
        ...
        if (state is NoLoggedInState) {
          ...
          downloadAndSaveImage(
              CandidateServices.getCandidateAvatarLink(_candidate.avatar!),
              _candidate.avatar!);
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Text(
              '${_candidate.lastName} ${_candidate.firstName}',
      ...
      Container(
        ...
        child: Column(
          children: [
            ...
            _buildPersonalInfoItem(
                AssetConstants.iconLocation, _candidate.location!),
            SizedBox(
              height: ValueConstants.deviceHeightValue(uiValue: 10),
            ),
            _buildPersonalInfoItem(
              ...
              Navigator.pushReplacement(
                ...
                MaterialPageRoute(
                    ...
                    settings: RouteSettings(arguments: {
                      TextConstants.candidate: _candidate,
                      TextConstants.token: _token,
                      TextConstants.avatarPath: _localAvatarPath,
                      TextConstants.emailUser: email,
                      TextConstants.passwordUser: password
                    })),
              );
            },
          )
